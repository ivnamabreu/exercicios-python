...
A função recebe uma lista (`lista`) e um valor (`item`). Remova a **primeira ocorrência** desse item e retorne a lista resultante. Se o item não estiver na lista, retorne a lista sem nenhuma alteração.
...
- `resposta(['a', 'b', 'a'], 'a')` deve retornar `['b', 'a']`
- `resposta(['x', 'y'], 'z')` deve retornar `['x', 'y']`
